...
const ProjectCard = ({ project, index, onStar, onDelete, onRename, onDuplicate, isStarred }) => {
    ...
    return (
        <div 
            ...
            className="bg-white border border-slate-100 rounded-xl p-6 cursor-pointer hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-200/40 transition-all duration-300 flex flex-col shadow-sm group relative"
        >
            <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className={`p-1.5 rounded-md hover:bg-slate-50 transition-colors ${isStarred ? 'text-yellow-400' : 'text-slate-300 hover:text-slate-500'}`}
            ...
            <div className="mt-auto pt-4 border-t border-slate-50 flex justify-between items-center text-xs text-slate-400 font-medium">
                ...
                <div className="relative" ref={menuRef}>
                    <button 
                        ...
                        className="hover:text-slate-700 p-1 rounded-md hover:bg-slate-50"
                    >
                        <i className="ri-more-2-fill text-lg"></i>
                    </button>

                    {isMenuOpen && (
                        <div className="absolute right-0 bottom-8 w-40 bg-white rounded-lg shadow-xl border border-slate-100 py-1 z-10 animate-fade-in origin-bottom-right">
                            <button 
